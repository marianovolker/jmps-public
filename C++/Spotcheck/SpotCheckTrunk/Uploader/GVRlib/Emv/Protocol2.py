"""
.. module:: GVRlib.Emv.Protocol2
   :platform: Unix, Windows
   :synopsis: EMV protocol management (Python 2).
...
EMV_MULTI_BER_TLV_EXAMPLE = "".join(EMV_BER_TLV_EXAMPLE)
...
def recv_START_EMV_cmd(connection, timeout = 0):
	...
	"""
	return recv_EMV_ACK(connection, '\x01', timeout)
...
def recv_ACTION_EMV_cmd(connection, timeout = 0):
	"""
	Receives an APDU containing the answer produced by the SPOT on the **ACTION_EMV** *(0x01)* command.
	...
	"""
	return recv_EMV_ACK(connection, '\x02', timeout)
...
def recv_OLA_EMV_cmd(connection, timeout = 0):
	...
	"""
	return recv_EMV_ACK(connection, '\x03', timeout)
...
def recv_END_EMV_cmd(connection, timeout = 0):
	...
	"""
	return recv_EMV_ACK(connection, '\x04', timeout) 
...
def recv_START_EMV_1_cmd(connection, timeout = 0):
	...
	"""
	return recv_EMV_ACK(connection, '\x05', timeout)
...
def send_START_EMV_2_cmd(connection, nextCommand = 0):
	...
	:Parameters:
		- *connection* (PupySPOT): Object managing spot connection.
		- nextCommand (byte):
		    - 0x00 = like START_EMV_2 without otional flag,
...
def recv_START_EMV_2_cmd(connection, timeout = 0):
	...
	"""
	return recv_EMV_ACK(connection, '\x06', timeout) 
...
def send_START_EMV_2_PHASE_2_cmd(connection, amount = 0):
	...
	"""
	apdu = pupyspot.create_apdu(_apid, '\x10\x07')
	apdu.put_dword(amount)
	connection.send_apdu(apdu)
...
def recv_START_EMV_2_PHASE_2_cmd(connection, timeout = 0):
	...
	"""
	return recv_EMV_ACK(connection, '\x07', timeout)
...
def recv_EMV_ACK(connection, subcommand, timeout = 0):
	...
	"""
	apdu = connection.wait_apdu(0x80 + _apid, '\x10' + subcommand, timeout)
...
def wait_EMV_Unsolicited(connection, timeout = 0):
	...
	"""
	apdu = connection.wait_apdu(0x80 + _apid, '\x99\x02', timeout)
...
def send_read_tags_cmd(connection, *tags):
	...
	tags_length = 0
	for tag in tags:
		tags_length = tags_length + len(tag)
	apdu.put_word(tags_length)
	for tag in tags:
		apdu.put_array(tag)
	connection.send_apdu(apdu)
...
def send_table_dump_load_cmd(connection, table):
	"""
	Sends a **Table Dump Load** command to SPOT. 
...
def send_dump_load_cmd(connection):
	...
	"""
	apdu = pupyspot.create_apdu(_apid, '\x20\x06\xff\x02')
	connection.send_apdu(apdu)
