"""
.. module:: GVRlib.OPT.Protocol3 
   :platform: Unix, Windows
   :synopsis: OPT protocol management (Python 3).
...
def send_oti_enable_cmd(connection, transaction_amount = None, transaction_data = None, transaction_type = None, transaction_time = None):
    ...
    """
    apdu = pupyspot.create_apdu(_apid, b'\x50\x01')
    # Put Application Mask for backward compatibility
    apdu.put_word(0x0000)
    apdu.put_array(transaction_amount if transaction_amount else b'\x00\x00\x00\x00\x11\x22')
    apdu.put_array(transaction_data if transaction_data else b'\x10\x01\x11')
    apdu.put_byte(transaction_type if transaction_type else 0x00)
    apdu.put_array(transaction_time if transaction_time else b'\x00\x00\x00')
    connection.send_apdu(apdu)
...
def send_oti_disable_cmd(connection):
    ...
    :Parameters:
        - *connection* (PupySPOT): Object managing spot connection.
    """
    apdu = pupyspot.create_apdu(_apid, b'\x50\x05')
    connection.send_apdu(apdu)
...
def send_job_status(connection, jobID):
    ...
    :Parameters:
        ...
        - *jobID* (dword): Unique job identifier.
    """
    apdu = pupyspot.create_apdu(_apid, b'\x20\x03')
    apdu.put_dword(jobID)
    connection.send_apdu(apdu)
...
def send_execute_job(connection, jobID):
    ...
    :Parameters:
        - *connection* (PupySPOT): Object managing spot connection. 
    """
    apdu = pupyspot.create_apdu(_apid, b'\x20\x05')
    apdu.put_dword(jobID)
    connection.send_apdu(apdu)
...
def recv_oti_enable_cmd(connection, timeout = 0):
    ...
    :Returns: A dictionary containing the "answer APDU" parameters. 
    """
    apdu = connection.wait_apdu(0x80 + _apid, b'\x50\x01', timeout)
    ack_code = apdu.pop_byte()
    ack_params = {
            'ack_code': ack_code
        }
...
def recv_oti_track_data_cmd(connection, timeout = 0):
    ...
    :Returns: A dictionary containing the "answer APDU" parameters. 
    """
    apdu = connection.wait_apdu(0x80 + _apid, b'\x50\x04', timeout)
    ack_code = apdu.pop_byte()
    ack_params = {
            'ack_code': ack_code
        }
...
def recv_execute_job(connection, timeout = 0):
    ...
    :Returns: A dictionary containing the "answer APDU" parameters. 
    """
    apdu = connection.wait_apdu(0x80 + _apid, b'\x20\x05', timeout)
    ack_code = apdu.pop_byte()
    if ack_code == 0x00:
        ack_params = {
            'ack_code': ack_code,
