...
"""
.. module:: GVRlib.Pinpad.Utility2
   :platform: Unix, Windows 
   :synopsis: Pinpad utility module (Python 2).
...
def simmetric_keydata_generator(key_use, key_type, key_data, transport_key, SMIDR = ''):
	"""
	...
	return crypto.encrypt('DES3_CBC', keyblock, transport_key) + ('' if SMIDR is None else SMIDR)

def simmetric_keydata_unpack(keyblock, transport_key = '\x37\x36\x35\x34\x33\x32\x31\x30\x37\x36\x35\x34\x33\x32\x31\x30'):
        """
        TODO.
	...
	print('KEYBLOCK: ' + hexlify(dec_keyblock))
	print('   RAND : ' + hexlify(dec_keyblock[:2]))
	print('   TYPE : ' + hexlify(dec_keyblock[2]))
	print('   USE  : ' + hexlify(dec_keyblock[3]))
	print('   VALUE: ' + hexlify(dec_keyblock[4:-4]))
	print('   CRC32: ' + hexlify(dec_keyblock[-4:]))
	print('   SMIDR: ' + hexlify(SMIDR))
...
def DeleteKeyM3(connection, key_id):
        ...
        """
	apdu = pupyspot.create_apdu(0x01, str(key_id) + '\x01\xFF\x03\x00\x00\x00\x00\x21\x44\xDF\x1C')
...
def calc_data_encrypt(key, data, type, versus, SMIDR = None, iv = None):
	...
	if type is Enums.KeyKind.DES:
		algorithm = 'DES'
		if iv is None: iv = '\x00' * crypto.block_size
	elif type is Enums.KeyKind.TDES:
		algorithm = 'DES3'
		if iv is None: iv = '\x00' * crypto.block_size
	elif type is Enums.KeyKind.AES:
		algorithm = 'AES'
		if iv is None: iv = '\x00' * crypto.iv_size
...
class Resource_File(object):
        ...
        """
	def __init__(self, file_path, file_type = 0, resource_id = 0, auth_flag = 0, description = ''):
...
def tr31_kba(kbpk, key, header):
	...
	kbmk = ''
	for byte in kbpk: 
		kbek = kbek + chr(0x45 ^ ord(byte))
	for byte in kbpk: 
		kbmk = kbmk + chr(0x4D ^ ord(byte))
	key_len = pack('!H', len(key) * 8)
	if key_len == '\x00\x80': rand_bytes = os.urandom(6)
	if key_len == '\x00\x40': rand_bytes = os.urandom(14)
	key_block = key_len + key + rand_bytes
	enc_key_block = crypto.encrypt("DES3_CBC", key_block,  kbek, iv = header[:8])
	mac = mac_cbc(kbmk, str(header) + str(enc_key_block))
	return header + hexlify(enc_key_block + mac)

def tr31_header(block_version, key_usage, algorithm, use_mode, version_num, exportability, optional_block = '0000'):
        """
        TODO.
    
        """
	kb_lenght = '%04d' % (68 + len(optional_block))
	header = pack('!c4s2scc2sc', block_version, kb_lenght, key_usage, algorithm, use_mode, version_num, exportability) + optional_block
	return header
...
def makeTerminalCAcertificate(cert_request):
	...
	try:
		...
		if req.verify(pub_key):
			...
			newCertificate.set_serial_number(long(hexlify(os.urandom(64)), 16))
			newCertificate.set_issuer(issuer)
			from datetime import datetime, timedelta
			now = (datetime.now() - timedelta(days = 7)).strftime("%Y%m%d%H%M%SZ")
			expire  = (datetime.now() + timedelta(days = 365)).strftime("%Y%m%d%H%M%SZ")
			...
			len_term_cert = len(terminal_certificate)
			len_term_cert = chr(len_term_cert >> 8) + chr(len_term_cert & 0xff)
			len_cama_cert = len(masterCA_certificate)
			len_cama_cert = chr(len_cama_cert >> 8) + chr(len_cama_cert & 0xff)
			mac_data = len_term_cert + terminal_certificate + len_cama_cert + masterCA_certificate;
			import hashlib
			sha256_hash = hashlib.sha256(mac_data).digest()
			mac = crypto.mac_x919(sha256_hash, '0123456701234567');
			return (long(hexlify(mac), 16), terminal_certificate, masterCA_certificate)
		else:
