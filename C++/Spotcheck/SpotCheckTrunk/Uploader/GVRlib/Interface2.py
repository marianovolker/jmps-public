...
def login(connection, client_id = _client_id, ping_timeout = _ping_timeout, type = _type, modulo_len = 0, verbose = True, ack_expected = 0x00):
	...
	if ack_params['ack_code' ] == ack_expected:
		if verbose:
			...
			print(' - Date&Time: ' + ack_params['date_time'])
			entries = ack_params['entries']
			if len(entries) > 0:
				for entry in ack_params['entries']:
					print(' - AppID: ' + str(entry['app_id']) + ' Sign: ' + hex(entry['sign']) + ' AppAscii: ' + entry['app_ascii'])
			else:
			...
			if pmk is None:
				...
				print(' - PMK Module: ' + pmk['rsa_mod'] + ' Exponent: ' + pmk['rsa_exp'])
			if 'fwr_info' in ack_params:
				print(' - FWR Info: ' + ack_params['fwr_info'])
			if 'hwr_info' in ack_params:
				print(' - HWR Info: ' + ack_params['hwr_info'])
...
def send_login_request(connection, client_id = _client_id, ping_timeout = _ping_timeout, type = _type, modulo_len = 0):
	...
	"""
	apdu = pupyspot.create_apdu(_apid, '\x01')
...
def send_logout_request(connection, client_id = _client_id):
	...
	"""
	apdu = pupyspot.create_apdu(_apid, '\x02')
	connection.send_apdu(apdu)
...
def send_ping(connection):
	...
	"""
	apdu = pupyspot.create_apdu(_apid, '\x03')
	connection.send_apdu(apdu)
...
def send_lock(connection, application_id):
	...
	"""
	apdu = pupyspot.create_apdu(_apid, '\x04')
...
def send_unlock(connection, application_id):
	...
	"""
	apdu = pupyspot.create_apdu(_apid, '\x05')
	apdu.put_byte(application_id)
	connection.send_apdu(apdu) 
...
def send_reboot(connection):
	...
	"""
	apdu = pupyspot.create_apdu(_apid, '\x06')
	connection.send_apdu(apdu)
...
def send_reboot_kernel_EMV(connection):
	...
	"""
	apdu = pupyspot.create_apdu(_apid, '\x07')
	connection.send_apdu(apdu)
...
def send_init_log(connection, process_name):
	...
	""" 
	apdu = pupyspot.create_apdu(_apid, '\x08')
...
def send_log_line(connection, log_type, datalib):
	...
	"""
	apdu = pupyspot.create_apdu(_apid, '\x09')
...
def recv_login_request(connection, timeout=0):
    ...
    """
	apdu = connection.wait_apdu(0x80 + _apid, '\x01', timeout)
	...
	if ack_code == 0x00: 
		...
		for i in range(0, ack_params['num_apps']):
			...
			ack_params['entries'].append(entrie)
...
def recv_logout_result(connection, timeout=0):
    ...
    """
	apdu = connection.wait_apdu(0x80 + _apid, '\x02', timeout)
	ack_params = {
				'ack_code' : apdu.pop_byte()
				}
	return ack_params
...
def recv_ping_result(connection, timeout=0):
    ...
    """
	apdu = connection.wait_apdu(0x80 + _apid, '\x03', timeout)
	ack_params = {
				'ack_code' : apdu.pop_byte()
				}
	return ack_params
...
def recv_lock(connection, timeout=0):
    ...
    """
	apdu = connection.wait_apdu(0x80 + _apid, '\x04', timeout)
	ack_params = {
				'ack_code' : apdu.pop_byte()
				}
	return ack_params
...
def recv_unlock(connection, timeout=0):
    ...
    """
	apdu = connection.wait_apdu(0x80 + _apid, '\x05', timeout)
	ack_params = {
				'ack_code' : apdu.pop_byte()
				}
	return ack_params
...
def recv_reboot_kernel_EMV(connection, timeout=0):
    ...
    """	
	apdu = connection.wait_apdu(0x80 + _apid, '\x07', timeout)
	ack_params = {
				'ack_code' : apdu.pop_byte()
				}
	return ack_params
...
def recv_init_log(connection, timeout=0):
    ...
    """	
	apdu = connection.wait_apdu(0x80 + _apid, '\x08', timeout)
	ack_params = {
				'ack_code' : apdu.pop_byte()
				}
	return ack_params
...
def recv_log_line(connection, timeout=0):
    ...
    """
	apdu = connection.wait_apdu(0x80 + _apid, '\x09', timeout)
	ack_params = {
				'ack_code' : apdu.pop_byte()
				}
	return ack_params
...
def recv_diagnostic(connection, timeout=0):
    ...
    """
	apdu = connection.wait_apdu(0x80 + _apid, '\x0F', timeout)
	ack_params = {
...
def recv_session_keys(connection, timeout=0):
    ...
    """
	apdu = connection.wait_apdu(0x80 + _apid, '\x11', timeout)
	ack_params = {
				'ack_code' : apdu.pop_byte()
				}
	return ack_params
...
def recv_maintenance_request(connection, timeout = 0):
	apdu = connection.wait_apdu(0x80 + _apid, '\x00\x0c', timeout)
	ack_code = apdu.pop_byte()
	if ack_code == 0x00:
		ack_params = {
				'ack_code': 0x00,
...
if __name__ == '__main__':
	...
	def loginTest(connection, client_id, modulo_len, ping_timeout, ack_expected,dologout, verbose, silence = False):
		...
		if ack_params['ack_code' ] == ack_expected:
			...
			if verbose == True and silence == False:
				try:
					print(' - Date&Time: ' + ack_params['date_time'])
					for entrie in ack_params['entries']:
						print(' - AppID: ' + str(entrie['app_id']) + ' Sign: ' + hex(entrie['sign']) + ' AppAscii: ' + entrie['app_ascii'])
					if pmk is None:
						print('No PMK key in Login answer')
					else:
						print(' - PMK Module: ' + pmk['rsa_mod'] + ' Exponent: ' + pmk['rsa_exp'])
	...
	while dist not in [ 'P', 'D' ]:
		dist = raw_input('(P)rod / (D)evel: ')

	cycle = ''
	while cycle not in [ 'Y', 'N' ]:
		cycle = raw_input('Do you want to execute test with all possible value (where is possible)? (Y)es / (N)o: ')
