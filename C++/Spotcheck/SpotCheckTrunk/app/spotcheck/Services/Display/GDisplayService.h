...
const int    DY_GET_WINDOW_COUNT_CMD_LEN  =  6;
...
public:
	...
	inline GDisplayServiceStatistics & GetStatistics() {return m_Statistics;}
						   ...
						   inline  int GetStoredTotalWindowsOnSPOT();
						           int UpdateTotalWindowsOnSPOT();
						          bool ProcessWindowCountReply(UINT8* pData, UINT uLength);
						          bool ProcessDisplayReply(GSPOTMessage* pMsg);
						   static bool isDisplayReply(GSPOTMessage* pMsg, bool blOnlySubCmd = false);
...
private:
				   ...
				   static const UINT8  mst_aryu8GetWindowCountCmd[DY_GET_WINDOW_COUNT_CMD_LEN];


			   static GDisplayService* mst_pSoleInstance;
						 static GMutex mst_createInstanceMutex;

								int    m_iTotalWndOnSPOT;
...
inline int GDisplayService::GetStoredTotalWindowsOnSPOT()
{
	GMutexLock ml(m_instanceMutex);

	return m_iTotalWndOnSPOT;
}
