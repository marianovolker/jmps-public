...
TEST_F(GCardReaderServiceUT, TestCardCycle)
{
	...
	m_pService->ProcessSystemStatusNotification(0x01, 0x80, 0); // card outside and CR enabled.
	EXPECT_TRUE( m_pService->GetStatus() == GCardReaderService::eStatusDataArrived ); // ignored

	m_pService->ProcessSystemStatusNotification(0x00, 0x80, 0); // card outside and CR disabled.
	EXPECT_TRUE( m_pService->GetStatus() == GCardReaderService::eStatusCardOut ); // card outside
...
TEST_F(GCardReaderServiceUT, TestFLRChipCardCycle)
{
	UINT8  aryu8CardReaderMsg[48];
	UINT8  aryu8ATR[6];
	...
	m_pService->ProcessSystemStatusNotification(0x01, 0x80, 0); // CR enabled notification
	EXPECT_TRUE( m_pService->GetStatus() == GCardReaderService::eStatusCardOut ); // card outside status

	m_pService->ProcessSystemStatusNotification(0x01, 0x83, 0);
	EXPECT_TRUE( m_pService->GetStatus() == GCardReaderService::eStatusChipCardInserted ); // Card Inserted status

	GConversion::ASCIIHexStringToBytes(aryu8CardReaderMsg,
		"00 12 81 00 20 03 00 01 08 00 00 06 3B 62 00 00 81 82");

	GSPOTMessage oMsg(aryu8CardReaderMsg);
	m_pService->ProcessCardReaderReply(&oMsg);
	EXPECT_TRUE( m_pService->GetStatus() == GCardReaderService::eStatusATRDataArrived ); // ATR Arrived status

	GConversion::ASCIIHexStringToBytes(aryu8ATR, "3B 62 00 00 81 82");
	EXPECT_TRUE( memcmp(m_pService->GetTrackInfo(eTrack4).m_aryu8Data, aryu8ATR, sizeof(aryu8ATR)) == 0 );

	m_pService->ProcessSystemStatusNotification(0x02, 0x83, 0); // CR busy processing
	EXPECT_TRUE( m_pService->GetStatus() == GCardReaderService::eStatusATRDataArrived ); // ATR Arrived status
	...
	m_pService->ProcessSystemStatusNotification(0x01, 0x80, 0); // premature card remove
	EXPECT_TRUE( m_pService->GetStatus() == GCardReaderService::eStatusATRDataArrived ); // ATR Arrived status
