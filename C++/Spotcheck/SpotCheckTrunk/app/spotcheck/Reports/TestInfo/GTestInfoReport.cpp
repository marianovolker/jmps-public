...
const char* GTestInfoReport::mst_arypcTextLines_vgd[TESTREPO_MAX_LINES_VGD] =
{
	"-------------------------------\n", // line  0
	"Card Reader Statistics\n",          // line  1
	"\n",                                // line  2
...
const char* GTestInfoReport::m_arpcTRSLEDLabels[]= {
		"No Test",
		"Pass   ",
		"Fail   ",
};
...
int GTestInfoReport::TextDump()
{
	...
	{
		return this->TextDumpVGD(GReportHeader::eTestResultsReport);
	}
...
std::string GTestInfoReport::GetKeysString(GKeypadService::eKeyValue e1, GKeypadService::eKeyValue e2)
{
	...
	for(int i = e1; i <= e2; ++i)
	{
		...
		if (u8times != 0xFF && u8times > 0)
		{
			...
			{
				strOutput += m_pKPService->GetKeyLabel(UINT8(i) -(UINT8)GKeypadService::eKeyL4);
			}
			else
			{
...
int GTestInfoReport::TextDumpVGD(GReportHeader::eReportType eType)
{
	GCardReaderService* pCRSvc = GCardReaderService::GetService();
	GDisplayService* pDYSvc = GDisplayService::GetService();
	const GCardReaderServiceStatistics & rCRStats = pCRSvc->GetSessionStatistics();
	...
	// Fill auxiliary keys
	sKeys = GetADAKeysString(); // line 33
	PopulateLine(nLine++, sKeys.c_str());
	if (NoMoreRoomOnBuffer(64))
		return GetPartialCount();
	...
	while (nLine < 51)  // lines 47 to 50
		PutVoidDataLine(nLine++);

	//Feature Connector
	PopulateLine(nLine++, GFeatureConnectorStatistics::GetInstance()->GetExternalStatistics()); //line 51
	PopulateLine(nLine++, GFeatureConnectorStatistics::GetInstance()->GetInternalNsStatistics()); //line 52

	while (nLine < 57)  //lines 53 to 56
		PutVoidDataLine(nLine++);

	//TRIND Statistics
	GTrindService* pTRService = GTrindService::GetService();
	GTrindServiceStatistics& rTRStats = pTRService->GetSessionStatistics();

	UINT32 u32TrPass  = rTRStats.GetTagStatistics(GTrindServiceStatistics::eTrPass);
	UINT32 u32TrFail  = rTRStats.GetTagStatistics(GTrindServiceStatistics::eTrFail);
	UINT32 u32TrTotal = u32Pass + u32Fail;
	UINT32 u32TrLED   = rTRStats.GetLEDStatistics();

	PopulateLine(nLine++, m_arpcTRSLEDLabels[rTRStats.GetLEDStatistics()]); //line 57

	while(nLine < 60)
		PutVoidDataLine(nLine++);                       //line 58, 59

	PopulateLine(nLine++, u32Pass);                     //line 60
	PopulateLine(nLine++, u32Fail);                     //line 61
	PopulateLine(nLine++, u32Total);                    //line 62

	while(nLine < 66)
		PutVoidDataLine(nLine++);                       //line 63 -65

	// Secure Audio Results
	GKeypadService* pKpService = GKeypadService::GetService();
	PopulateLine(nLine++, pKpService->GetSAResults());   //line 66

	PopulateLine(nLine, 0x1B, 'E'); // line 67: paper advance escape character
	return GetPartialCount();
}
