...
const char*  GReportHeader::mst_arypcVgdHeaderLines[HEADER_MAX_LINES] =         //  for VGD platform.
{
	"\n",                                // line  0
	"%19.19s\n",                         // line  1
	"SPOTCheck %9s rev. %s\n",           // line  2
...
int GReportHeader::PopulateLine(int nLine, ...)
{
	...
	{
		iCharsWritten = vsnprintf(m_pcOut, mst_iMaxLineLength,mst_arypcHeaderLines[nLine], args);
	}
	else
	{
		iCharsWritten = vsnprintf(m_pcOut, mst_iMaxLineLength,mst_arypcVgdHeaderLines[nLine], args);
	}
