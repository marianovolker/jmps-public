...
const char* GSoftKeysWindowTemplate::m_pcTimerInitLabel = "Auto Exit\n180 secs.";
const char* GSoftKeysWindowTemplate::m_pcTimerLabelKey  = "Auto Exit\n%d secs.";
...
void GSoftKeysWindowTemplate::InitializeTagList()
{
	   ...
	   for ( int i=0; i < eTOTAL_TAGS; i++)
	   {
	      if (new_seg != eSEGMENT_9)
	      {
	    	 if( new_seg == eTIMER)
	    	 {
	    		 m_TagMap[new_seg] = std::string(m_pcTimerInitLabel);
	    	 }
	    	 else
	    	 {
...
void GSoftKeysWindowTemplate::BuildTextTag(std::string text, eTAG_ID TagId)
{
   ...
   if(TextTag->m_u8TagId == eTIMER)
   {
	   TextTag->m_u8BorderSize = 2;
	   TextTag->m_BorderRGB.Red = 255; // 0xFF;
	   TextTag->m_BorderRGB.Green = 165;
...
void GSoftKeysWindowTemplate::GetTagPosition(eTAG_ID TagId, UINT16& XOrg, UINT16& YOrg)
{
   switch (TagId)
   {
      ...
      case eSOFT_L4:
    	  	  	  	  ...
    	  	  	  	  {
    	  	  	  		  YOrg = (TagId - 1) * 58 + 22;  // for M5 and VGD
    	  	  	  	  }
    	  	  	  	  else
    	  	  	  	  {
    	  	  	  		  YOrg = (TagId - 1) * 60;
      ...
      case eSOFT_R4:
         	 	 	 	 ...
         	 	 	 	 if(!GGlobalSystemInformation::GetInstance()->isM3Unit())
         	 	 	 	 {
         	 	 	 		YOrg = (TagId % eSOFT_R1) * 58 + 22;      // for M5 and VGD
         	 	 	 	 }
         	 	 	 	 else
         	 	 	 	 {
         	 	 	 		YOrg = (TagId % eSOFT_R1) * 60 + 15;
         	 	 	 	 }
