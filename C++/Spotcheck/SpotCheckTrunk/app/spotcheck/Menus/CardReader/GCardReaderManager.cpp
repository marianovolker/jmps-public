...
void GCardReaderManager::NotifyProcessingCard()
{
	LOG(eCardReader, eDebug, "GCardReaderManager::NotifyProcessingCard().CARD INSERTED\n");

	if (m_pCRMenu)
	{
		SetStatus(eShowProcessingCard);
		m_pCRMenu->ClearScreen();
		m_pCRMenu->ShowProcessingCard();
	}
}
...
bool GCardReaderManager::ChangeStateTo(eStatus windowStatus)
{
	...
	switch (windowStatus)
	{
		...
		case eShowTestResult:
			...
			if (m_Status == eShowInitial || m_Status == eShowProcessingCard || m_Status == eShowPleaseRemoveCard || m_Status == eShowDetails || m_Status == eShowStats)
			{
