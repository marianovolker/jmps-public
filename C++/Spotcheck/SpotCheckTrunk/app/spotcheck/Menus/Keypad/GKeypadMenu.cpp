...
int GKeypadMenu::GetTagStrings(GSoftKeysWindowTemplate::tStringTagMap & rMap)
{
	...
	snprintf(arycBuffer, sizeof(arycBuffer), "Hardware ID:  %02X%02X  ", int(rKBInfo.m_aryu8HwrID[0]), int(rKBInfo.m_aryu8HwrID[1]));
	...
	snprintf(arycBuffer, sizeof(arycBuffer), "S/N:  %s  ", rKBInfo.m_pcSerialNumber);
