...
void GMenuManager::ProcessInactiveTimeout(UINT32 tLeft)
{
	if( !m_MenuStack.empty() )
		{
			m_MenuStack.top()->HandleInactiveTimeEvent(tLeft);
		}
}
...
bool GMenuManager::isServiceMessage(GSPOTMessage* pMessage)
{
	...
	if( pMessage->GetAppId() == eSPOT_PP_ANSWER )
	{
		...
		if( pMessage->GetCommandCode() == eSPOT_PP_DISPLAY )
			return true;
	...
	else if( pMessage->GetAppId() == eSPOT_OPT_ANSWER )
	{
		...
		if( pMessage->GetCommandCode() == eSPOT_OPT_TRIND )
				return true;
	}
...
bool GMenuManager::isSecureMediaConnectorMessage(GSPOTMessage* pMessage)
{
	LOG(eMenuManager, eDebug, "GMenuManager:isSecureMediaConnectorMessage \n");
	if((pMessage->GetCommandCode() == eSPOT_PP_DISPLAY) &&
		(pMessage->GetSubCommandCode()== eSPOT_PP_DY_SECUREMEDIACONNECTOR))
	{
		LOG(eMenuManager, eDebug, "isSecureMediaConnectorMessage :YES\n");
		return true;
	}
	LOG(eMenuManager, eDebug, "isSecureMediaConnectorMessage :NO\n");
	return false;
...
void GMenuManager::ProcessMessage(GSPOTMessage* pMessage)
{
	...
	{
		if(GGlobalSystemInformation::isUPMMessageReply(pMessage))
		{
			GGlobalSystemInformation* pSysInfo = GGlobalSystemInformation::GetInstance();
			pSysInfo->ParseUPMMessageReply(pMessage);
			return;
		}
	}
	...
	if(GGlobalSystemInformation::GetInstance()->isVGDUnit())
	{
		if(GGlobalSystemInformation::isPkgInfoMessageReply(pMessage))
		{
			GGlobalSystemInformation* pSysInfo = GGlobalSystemInformation::GetInstance();
			pSysInfo->ParsePkgInfoReply(pMessage);
			return;
		}
	}
	...
	if( isServiceMessage(pMessage) && !isSecureMediaConnectorMessage(pMessage))
	{
	...
	if( !m_MenuStack.empty() )
	{
		    ...
		    && (pMessage->GetSubCommandCode() == eSPOT_OPT_IO_KEYENABLE) )
		{
			...
			if (data[0] == 0x00 /* ack code ok */)
			{
				...
				m_MenuStack.top()->ResetTimerLabel();
				m_MenuStack.top()->HandleSoftKeyEvent(data[1]); // received keycode
				if (!m_bShouldPopMenu)
				{
					m_MenuStack.top()->DrawMenu();
				}
			...
			{
				LOG(eMenuManager, eWarning,
				    "GMenuManager::ProcessMessage - Soft Key ACK code error: %02X \n",
				    data[0]);
			}
	...
	while( iItemNo < nItems && (pb - aryu8NotificationData) < u16NotificationLength )
	{
		...
		else if( pMsg->GetAppId() == eSPOT_OPT_ANSWER )
		{
			switch( iDeviceCode )
			{
				...
				case eSPOT_SYS_TRIND:
					bl = GTrindService::GetService()->ProcessOPTSystemStatusNotification(u8Status, u8StatusEx, u8Error);
					blret = blret || bl;
					break;
...
bool GMenuManager::ProcessDiagnosticInfoNotification(GSPOTMessage* pMsg)
{
	...
	// The purpose of the next block of code is for printing in debugging ONLY.
/*
...
bool GMenuManager::ProcessServiceMessage(GSPOTMessage* pMsg)
{
	...
	if( iAppId == eSPOT_PP_ANSWER ) // services talking with Pinpad application
	{
		switch (iDevice)
		{
			...
			case eSPOT_PP_DISPLAY:
				if (GDisplayService::isDisplayReply(pMsg, true))
				{
					blret = GDisplayService::GetService()->ProcessDisplayReply(pMsg);
				}
			break;
	...
	else if( iAppId == eSPOT_OPT_ANSWER )
	{
		switch (iDevice)
		{
			...
			case eSPOT_OPT_TRIND:
				if (GTrindService::isTrindReply(pMsg,true))
				{
					blret = GTrindService::GetService()->ProcessTrindReply(pMsg);
				}
			break;
