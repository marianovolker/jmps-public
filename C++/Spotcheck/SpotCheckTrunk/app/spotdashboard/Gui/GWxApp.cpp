...
void MyApp::SetOtherErrorsKP(bool bValue)
{
	SetTextError(bValue, frame->m_staticTextOtherErrorsKP);
}


void MyApp::SetOtherErrorsCR(bool bValue)
{
	SetTextError(bValue, frame->m_staticTextOtherErrorsCR);
}

void MyApp::SetOtherErrorsDY(bool bValue)
{
	SetTextError(bValue, frame->m_staticTextOtherErrorsDISPLAY);
}
...
bool MyApp::DisconnectAndConnect()
{
	GConectionThread* thread = new GConectionThread(frame);
    wxThreadError err = thread->Create();

    if (err != wxTHREAD_NO_ERROR)
    {
        LOG(eUnCategorized, eError, "Couldn't create thread!\n" );
        return false;
    }
    ...
    if (err != wxTHREAD_NO_ERROR)
    {
    	LOG(eUnCategorized, eError, "Couldn't run thread!\n" );
        return false;
    }
...
void MyApp::UpdateStatusConnection(bool bSuccess)
{
	if( bSuccess )
	{
...
void MyApp::RefreshDataOnComponents()
{
	...
	//OTHER ERRORS COMPONENTS
	bFlag = controller->UpdateOtherErrorsCR();
	SetOtherErrorsCR(bFlag);
	bFlag = controller->UpdateOtherErrorsKP();
	SetOtherErrorsKP(bFlag);
	bFlag = controller->UpdateOtherErrorsDY();
	SetOtherErrorsDY(bFlag);
...
int MyApp::OnExit()
{
	LOG(eUnCategorized, eDebug, "GSPOTDashboardMainGUIChild::OnExit()!.\n" );

	frame->SetClose(true);

	LOG(eUnCategorized, eInfo, "Operation canceled by user!.\n" );
...
void MyApp::GrayDisplayIfCommError(bool bValue)
{
	...
	else
	{
		...
		frame->m_staticTextOtherErrorsDISPLAY->SetForegroundColour(wxColour(191, 191, 191)); //gray light
		...
		frame->m_staticTextOtherErrorsDISPLAY->SetBackgroundColour(wxColour(191, 191, 191)); //gray light
...
void MyApp::GrayCRIfCommError(bool bValue)
{
	...
	else
	{
		...
		frame->m_staticTextOtherErrorsCR->SetForegroundColour(wxColour(191, 191, 191)); //gray light
		...
		frame->m_staticTextOtherErrorsCR->SetBackgroundColour(wxColour(191, 191, 191)); //gray light
...
void MyApp::GrayKPIfCommError(bool bValue)
{
	...
	else
	{
		...
		frame->m_staticTextOtherErrorsKP->SetForegroundColour(wxColour(191, 191, 191)); //gray light
		...
		frame->m_staticTextOtherErrorsKP->SetBackgroundColour(wxColour(191, 191, 191)); //gray light
