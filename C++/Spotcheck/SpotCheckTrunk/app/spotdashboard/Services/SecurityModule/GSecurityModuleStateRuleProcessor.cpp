...
void GSecurityModuleStateRuleProcessor::ProcessSystemStatusNotification(UINT8 u8Status, UINT8 u8StatusEx, UINT8 u8Error)
{
	...
	if( u8Status < eSPOT_SM_ERROR_STATE && u8Error == eSPOT_SM_NOERROR )
	{
	...
	if( u8Status == eSPOT_SM_INOPERATIVE_STATE )
	{
		m_bOffline = true;
	}

	if( u8Error > eSPOT_SM_NOERROR)
	{
		...
		// The following code is a warning happening in some devices ¿?, not a real error.
		else if( u8Error != eSPOT_SM_SECURITYMODULEERROR )
		{
			m_bOtherError = true;
		}
