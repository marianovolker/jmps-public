...
bool GLoggerConfig::Initialize()
{
	...
	if(m_iRunningMode == eEmbedded || m_iRunningMode == eInjected ||m_iRunningMode == eDummyEmbedded ||m_iRunningMode == eDummyInjected )
	{
...
void GLoggerConfig::SetLogFilename()
{
	...
	do
	{
		...
		{
			iFileIndex++;
			memset(arycFileIndex,0x00,sizeof(arycFileIndex));
			snprintf(arycFileIndex, sizeof(arycFileIndex), "%03d",iFileIndex);
			m_strLogFilename.replace(20,3,arycFileIndex);
		}
	}while(true);         // end of do-while.

	if (iFileIndex == 0)    // Need to create First log file(DIAGNOSTIC.YYYYMMDD.000)
	{
		return;
	}

	iFileIndex--;               // Points to last existing log file.
	memset(arycFileIndex,0x00,sizeof(arycFileIndex));
	snprintf(arycFileIndex, sizeof(arycFileIndex), "%03d",iFileIndex);
	m_strLogFilename.replace(20,3,arycFileIndex);

	CalcLogFileSize();
	size=GetLogFileSize(); // Returns the size of the current log file


	if(size>=MAX_SIZE)
	{
		iFileIndex++;      // Incrementing file index to create a new log file.
		snprintf(arycFileIndex, sizeof(arycFileIndex), "%03d",iFileIndex);
		m_strLogFilename.replace(20,3,arycFileIndex);
	}
}
...
unsigned long GLoggerConfig::GetLogFileSize()
{
