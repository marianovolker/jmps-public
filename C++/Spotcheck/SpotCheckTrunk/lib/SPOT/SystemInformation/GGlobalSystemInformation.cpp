...
UINT8  GGlobalSystemInformation::mst_aryu8KeypadFWRCommand[8]=
			{0x00, 0x08, 0x01, 0x00, 0x10, 0x03, 0x01, 0x13 };

UINT8  GGlobalSystemInformation::mst_aryu8PkgInfoCommand[7]=
			{0x00, 0x07, 0x01, 0x00, 0x60, 0x03, 0x00};

UINT8  GGlobalSystemInformation::mst_aryu8UPMAppVerCommand[8]=
			{0x00, 0x08, 0x01, 0x00, 0x10, 0x03, 0x01, 0x14};

UINT8  GGlobalSystemInformation::mst_aryu8UPMBuildVerCommand[8]=
			{0x00, 0x08, 0x01, 0x00, 0x10, 0x03, 0x01, 0x0F};

UINT8  GGlobalSystemInformation::mst_aryu8CRFwrCommand[8]=
			{0x00, 0x08, 0x01, 0x00, 0x10, 0x03, 0x01, 0x10};
	...
	m_Condition(&m_Mutex)
{
	...
	m_SoftwareInfo.m_vgdFirmwareVersion = new char[eFwrInfoLength * FWR_VERSION_NO_LEN];
	m_SoftwareInfo.m_pcSoftwareVersions = new char[eSwfInfoLength * SWR_VERSION_NO_LEN];
	m_SoftwareInfo.m_vgdOSVersion = new char[SWR_VERSION_NO_LEN];
	m_VGDPkgInfo.m_pkgs = new char[ePkgInfoLength * PKG_INFO_LEN];
}
...
void GGlobalSystemInformation::DestroyInternalReferences()
{
	...
	if(m_SoftwareInfo.m_vgdFirmwareVersion)
	{
		delete m_SoftwareInfo.m_vgdFirmwareVersion;
		m_SoftwareInfo.m_vgdFirmwareVersion = NULL;
	}


	if(m_SoftwareInfo.m_vgdOSVersion)
	{
		delete m_SoftwareInfo.m_vgdOSVersion;
		m_SoftwareInfo.m_vgdOSVersion = NULL;
	}

	if(m_VGDPkgInfo.m_pkgs)
	{
		delete m_VGDPkgInfo.m_pkgs;
		m_VGDPkgInfo.m_pkgs = NULL;
	}
...
bool GGlobalSystemInformation::isUPMMessageReply(GSPOTMessage* pMsg)
{
	if(!pMsg)
	{
		return false; // no message is supplied
	...
	if( pMsg->GetSubCommandCode() != eSPOT_OPT_SYS_SWR_INFO )
	{
		return false; // unexpected command code
	}
...
bool GGlobalSystemInformation::isPkgInfoMessageReply(GSPOTMessage* pMsg)
{
	if(!pMsg)
	{
		return false;
	...
	if( pMsg->GetCommandCode() != eSPOT_OPT_VGD_SYSTEM)
	{
		return false; // unexpected command category
	}
...
bool GGlobalSystemInformation::SendKeypadFWRCommandToSPOT()
{
	GSPOTMessage oMsg(mst_aryu8KeypadFWRCommand);
	return m_pSession->SendSPOTCommand( &oMsg );
}

bool GGlobalSystemInformation::SendPkgInfoCommandToSPOT()
{
	GSPOTMessage oMsg(mst_aryu8PkgInfoCommand);
	return m_pSession->SendSPOTCommand( &oMsg );
}


bool GGlobalSystemInformation::SendUPMAppVerCommandToSPOT()
{
	GSPOTMessage oMsg(mst_aryu8UPMAppVerCommand);
	return m_pSession->SendSPOTCommand( &oMsg );
}

bool GGlobalSystemInformation::SendUPMBuildVerCommandToSPOT()
{
	GSPOTMessage oMsg(mst_aryu8UPMBuildVerCommand);
	return m_pSession->SendSPOTCommand( &oMsg );
}

bool GGlobalSystemInformation::SendCRFwrCommandToSPOT()
{
	GSPOTMessage oMsg(mst_aryu8CRFwrCommand);
	return m_pSession->SendSPOTCommand( &oMsg );
}
...
bool GGlobalSystemInformation::SendSyncKeypadFWRCommandToSPOT()
{
	GMutexLock oLock(m_Mutex);
	if( !SendKeypadFWRCommandToSPOT() )
	{
		return false;
	}
	return (m_Condition.Wait(5000) != 0);
...
bool GGlobalSystemInformation::SendSyncPkgInfoCommandToSPOT()
{
	GMutexLock oLock(m_Mutex);
	if( !SendPkgInfoCommandToSPOT() )
	{
		return false;
	}
	return (m_Condition.Wait(5000) != 0);
...
bool GGlobalSystemInformation::SendSyncUPMAppVerCommandToSPOT()
{
	GMutexLock oLock(m_Mutex);
	if( !SendUPMAppVerCommandToSPOT() )
	{
		return false;
	}
	return (m_Condition.Wait(5000) != 0);
...
bool GGlobalSystemInformation::SendSyncUPMBuildVerCommandToSPOT()
{
	GMutexLock oLock(m_Mutex);
	if( !SendUPMBuildVerCommandToSPOT() )
	{
		return false;
	}
	return (m_Condition.Wait(5000) != 0);
...
bool GGlobalSystemInformation::SendSyncCRFwrCommandToSPOT()
{
	GMutexLock oLock(m_Mutex);
	if( !SendCRFwrCommandToSPOT() )
	{
		return false;
	}
	return (m_Condition.Wait(5000) != 0);
...
int GGlobalSystemInformation::ReceiveHrwInfoReply(GSPOTMessage* pMsg)
{
	...
	if(isVGDUnit())
	{
		m_HardwareInfo.m_KeypadInfo.m_pcSerialNumber=m_HardwareInfo.m_SPOTInfo.m_pcSerialNumber;
	}
...
int GGlobalSystemInformation::ParsePkgInfoReply(GSPOTMessage* pMsg)
{
	int iret=ReceivePkgInfoReply(pMsg);
	ReceivedCommandFromSPOT();
	return iret;
}
...
int GGlobalSystemInformation::ParseUPMMessageReply(GSPOTMessage* pMsg)
{
	int iret=ReceiveUPMMessageReply(pMsg);
	ReceivedCommandFromSPOT();
	return iret;
}

int GGlobalSystemInformation::ReceivePkgInfoReply(GSPOTMessage* pMsg)
{
	int len = pMsg->GetDataLen();
	if(len < 8)
	{
	...
	UINT8* pb = pu8Data + 3;
	ParsePkgInfoFields(pb,len);
	return 1;
}

void GGlobalSystemInformation::ParsePkgInfoFields(UINT8* pMsg, int len)
{
	int iLen = 0;
	unsigned char tempBuffer[5 * PKG_INFO_LEN]={'\0'};
	unsigned char AppName[PKG_INFO_LEN]={'\0'};
	unsigned char AppVer[PKG_INFO_LEN]={'\0'};
	char* tempPtr;
	int itemCount = 0;
	...
	while(len - iLen >= 0)
	{
		...
		strcpy((char*)tempBuffer,(char*)&pMsg[iLen]);
		iLen = iLen + strlen((char*)tempBuffer);
		iLen++;
	
		tempPtr = strstr((char*)tempBuffer,(char*)"APPLICATION_NAME");
		if(tempPtr)
		{			
			snprintf((char*)AppName,sizeof(AppName),"%s",(char*)&tempBuffer[strlen("APPLICATION_NAME")+1]);
		}
		else
		{
			tempPtr = strstr((char*)tempBuffer,(char*)"APPLICATION_VERSION");
			if(tempPtr)
			{				
				snprintf((char*)AppVer,sizeof(AppVer),"%s",(char*)&tempBuffer[strlen("APPLICATION_VERSION")+1]);
				strcat((char*)AppName, (char*)" ");
				if(!memcmp(AppName,"OS",2))
				{
					memset(tempBuffer,0x00,sizeof(tempBuffer));
					strcpy((char*)tempBuffer, (char*)AppName);
					strcat((char*)tempBuffer, (char*)AppVer);
					WriteCharField(tempBuffer, m_SoftwareInfo.m_vgdOSVersion, strlen((char*)tempBuffer), sizeof(AppName));
				}
				strcat((char*)AppName, (char*)AppVer);
				if(strlen((char*)AppName))
				{
					WriteCharField(AppName, &m_VGDPkgInfo.m_pkgs[itemCount * PKG_INFO_LEN], strlen((char*)AppName), PKG_INFO_LEN);
					memset(AppName,0x00,sizeof(AppName));
					memset(AppVer,0x00,sizeof(AppVer));
					itemCount++;
				}
			}
		}
...
int GGlobalSystemInformation::ReceiveUPMMessageReply(GSPOTMessage* pMsg)
{
	int len = pMsg->GetDataLen();
	if(len < 8)
	{
	...
	if(pu8Data[2] == 0x13)                          //    keypad FWR.
	{
		WriteCharField(pb, m_SoftwareInfo.m_vgdFirmwareVersion, len, len);
	}

	else if(pu8Data[2] == 0x14)                     //  UPM Application Version.
	{
		pb = pu8Data + 3;
		WriteCharField(pb, m_SoftwareInfo.m_vgdUPMAppVersion, len, len);
	}

	else if(pu8Data[2] == 0x10)                   // Card Reader Firmware version.
	{
		WriteCharField(pb, m_SoftwareInfo.m_vgdCRFwrVersion, len, len);
	}

	else if(pu8Data[2] == 0x0F)                    //   UPM Build Version.
	{
		WriteCharField(pb, m_SoftwareInfo.m_vgdUPMBuildVersion, len, len);
	}
	return 1;
}
	...
	switch(iItemID)
	{
		...
		case eSPOT_DY_DEVMODEL:
			...
			if( !strncmp(m_HardwareInfo.m_DisplayInfo.m_pcDevModel, "MCF5", 4) )
				...
				m_eSPOTModel = eM5;
			else if((!strncmp(m_HardwareInfo.m_DisplayInfo.m_pcDevModel, "UPM", 3)) ||
					(!strncmp(m_HardwareInfo.m_DisplayInfo.m_pcDevModel, "LCD module", 10)))
				m_eSPOTModel = eVGD;
			break;
...
int GGlobalSystemInformation::ReceiveHardwareConfigReply(GSPOTMessage* pMsg)
{
	...
	while( iItemNo < nItems && (pb - pu8Data) < len)
	{
		...
		if(iItemID == eSPOT_TRIND_HWRCFG)
		{
			iProcessedItems += UpdateTrindInfo(iItemID, iItemLen, pb);
		}
		pb += iItemLen; // now points to the following item's TLV structure
		iItemNo++;
	}
...
int GGlobalSystemInformation::UpdateTrindInfo(int iItemID, int iItemLen, UINT8* pData)
{
...
const char* GGlobalSystemInformation::GetSoftwareInfo(int index)
{
	...
	if(isVGDUnit())
	{
		return m_SoftwareInfo.m_vgdOSVersion;
	}
	else
	{
...
const char* GGlobalSystemInformation::GetPkgInfo(int index)
{
	if(index < eMinSwfIndex || index > eMaxSwfIndex)
		return NULL;

	return m_VGDPkgInfo.m_pkgs + index * PKG_INFO_LEN;
}

const char* GGlobalSystemInformation::GetSoftPkgInfo()
{
	return m_SoftwareInfo.m_pcSoftwareVersions;
}
...
const char* GGlobalSystemInformation::GetFirmwareInfo(int index)
{
	...
	if(isVGDUnit())
	{
		if(index==eCRFwrVersion)
		{
			return m_SoftwareInfo.m_vgdCRFwrVersion;
		}
		else
		{
			return m_SoftwareInfo.m_vgdFirmwareVersion;
		}
	}
