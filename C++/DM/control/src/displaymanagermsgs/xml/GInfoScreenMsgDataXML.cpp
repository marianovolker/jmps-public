...
const QString GInfoScreenMsgDataXML::CXMLTag					("infoscreen_request");
const QString GInfoScreenMsgDataXML::CXMLVersion				("interface_version");
const QString GInfoScreenMsgDataXML::CXMLData					("data");
const QString GInfoScreenMsgDataXML::CXMLSynchronousRequest		("SynchronizeMessage");

QByteArray GInfoScreenMsgDataXML::Marshall(GInfoScreenMsgData* pRequest)
{
	LOG_LEVEL4("Marshall()");
...
GInfoScreenMsgData* GInfoScreenMsgDataXML::UnMarshall(QByteArray & strXML) throw(GException)
{
	LOG_LEVEL4("UnMarshall()");

	GInfoScreenMsgData* poRequest = new GInfoScreenMsgData();
	QDomDocument oDoc;
	QDomNodeList oNodes;
	...
	try
	{
    	...
    	{
	    	throw GEXCEPTION(QString("Error: infoscreen_request opening tag not detected"));
    	}
