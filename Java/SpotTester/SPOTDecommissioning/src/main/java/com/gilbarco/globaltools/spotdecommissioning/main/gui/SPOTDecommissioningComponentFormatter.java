...
public class SPOTDecommissioningComponentFormatter
{
	...
	static
	{
		defaultBackgroundColor = SPOTDecommissioningDialogSettings.getBackgroundColor( );
		defaultComponentFont = SPOTDecommissioningDialogSettings.getcomponentFont( );
		defaultStatusBarFont = SPOTDecommissioningDialogSettings.getstatusBarFont( );
		defaultPanelTitleFont = SPOTDecommissioningDialogSettings.getPanelTitleFont( );
		defaultSmallFont = SPOTDecommissioningDialogSettings.getComponentSmallFont( );
		defaultButtonFont = SPOTDecommissioningDialogSettings.getButtonFont();
	}
	...
	public static Component setFont(Component c, Font f)
	{
		...
		for (int i = 0; i < ncomp; i++) // process every child component
		{
			...
			if (child instanceof Container)
				SPOTDecommissioningComponentFormatter.setFont( child, f ); // recursive
		...
		if (c instanceof Panel || c instanceof JPanel)
		{
			...
			for (int i = 0; i < ncomp; i++) // Process every child panel component, if any.
			{
				...
				if (child instanceof Panel || child instanceof JPanel)
																		SPOTDecommissioningComponentFormatter
