 ...
 * @date 03-May-2012
 */
public class SPOTLogCollectorComponentFormatter
{
	...
	static
	{
		defaultBackgroundColor = SPOTLogCollectorDialogSettings.getBackgroundColor( );
		defaultComponentFont = SPOTLogCollectorDialogSettings.getcomponentFont( );
		defaultStatusBarFont = SPOTLogCollectorDialogSettings.getstatusBarFont( );
		defaultPanelTitleFont = SPOTLogCollectorDialogSettings.getPanelTitleFont( );
		defaultSmallFont = SPOTLogCollectorDialogSettings.getComponentSmallFont( );
		defaultButtonFont = SPOTLogCollectorDialogSettings.getButtonFont();
	}
	...
	public static Component setFont(Component c, Font f)
	{
		...
		for (int i = 0; i < ncomp; i++) // process every child component
		{
			...
			if (child instanceof Container)
				SPOTLogCollectorComponentFormatter.setFont( child, f ); // recursive
		...
		if (c instanceof Panel || c instanceof JPanel)
		{
			...
			for (int i = 0; i < ncomp; i++) // Process every child panel component, if any.
			{
				...
				if (child instanceof Panel || child instanceof JPanel)
																		SPOTLogCollectorComponentFormatter
