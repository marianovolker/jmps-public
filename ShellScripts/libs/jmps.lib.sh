...
# parameters  : $1 the ip to validate.
# return      : C_TRUE if OK, otherwise C_FALSE.
#######################################################################################
validate_ip_address()
{
  local ip="$1"
	...
	if expr "$ip" 1>&2 2>/dev/null : '[0-9][0-9]*\.[0-9][0-9]*\.[0-9][0-9]*\.[0-9][0-9]*$' 1>&2 2>/dev/null
	then
	  for i in 1 2 3 4
	  do
	    if [ $(echo "$ip" | cut -d. -f$i) -gt 255 ] || [ $(echo "$ip" | cut -d. -f$i) -le 0 ]
	    then
	      log "1: no valid ip ($ip)"
	      return $C_NOK
	    fi
	  done
	else
	  log "2: no valid ip ($ip)"
	  return $C_NOK
	fi  
