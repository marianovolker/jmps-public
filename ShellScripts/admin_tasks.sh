...
obtain_system_information()
{
  ...
  if [ "$C_MSJ_UPS_IS_ON_BATTERY_STATE" != "" ] && [ "$C_UPS_NOTIFICATION" -eq 1 ] 
  then 
    ...
    then
      ...
      if test "$ups_status" == "COMMLOST"
      then
        ups_output=$(/etc/init.d/apcupsd restart 2>&1)
        err=$(expr $err + $?)
        log "ups_output=$ups_output; err=$err"
      fi      
    fi
  fi
